...
async def test_snapshot_and_upload_uploads_before_deleting_and_keeps_three(
    ...
    httpx_mock.add_response(
        method="GET",
        url=f"{GITHUB_API}/repos/{REPO}/releases/tags/state",
        json={
            "id": 100,
            "assets": [
                {"id": 1, "name": "state-20260101T000000Z.enc"},
                {"id": 2, "name": "state-20260102T000000Z.enc"},
                {"id": 3, "name": "state-20260103T000000Z.enc"},
            ],
        },
    )
    httpx_mock.add_response(
